...
//         //예) 10분 동안 사용되지 않으면 해당 캐시 데이터가 삭제, 다시 요청 시 새 데이터 받아오게 함.
...
//         //기본값은 3회 정도, 네트워크 오류 등 임시적인 문제를 보완할 수 있음음
//         //retry:3,

//         //initialData: 쿼리 실행 전 미리 제공할 초기 데이터 설정
//         //컴포넌트가 렌더링될 때 빈 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해줌

//         //파라미터가 변경될 때 이전 데이터를 유지하여 UI 깜빡임을 (Flicking)을 줄여준다.
//         // 예) 페이지네이션 시 페이지 전환 사이에 이전 데이터를 보여주여 사용자 경험을 향상
