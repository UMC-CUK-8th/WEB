...
function usePostLike(){
    return useMutation({
        ...
        await queryClient.cancelQueries({
            queryKey:[QUERY_KEY.detail,lp.lpId]
        });
        ...
        const me=queryClient.getQueryData<ResponseMyInfoDto>([QUERY_KEY.info]);
        ...
        //업데이트된 게시글데이터를 캐시에 저장
        //이렇게 하면 UI가 바로 업데이트됨, 사용자가 변화를 확인할 수 있다.
        queryClient.setQueryData([QUERY_KEY.detail,lp.lpId],newLpPost);
